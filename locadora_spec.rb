...
describe CarrinhoRegras do
  ...
  context "Caso exista um filme de Ação no carrinho deve ser aplicado mais 5% de deconto" do
    it "Existe filme de Ação no carrinho" do
      obj = CarrinhoRegras.new()
      ac = obj.valida_genero_acao(true)
      expect(val).to eq(0.5)
    end
    it "Existe filme de Ação no carrinho" do
      obj = CarrinhoRegras.new()
      ac = obj.valida_genero_acao(false)
      expect(val).to eq(0)
    end
  end
end
